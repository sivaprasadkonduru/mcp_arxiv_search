"""
MCP Chatbot — connects to remote servers via SSE or Streamable HTTP.

Usage:
    # 1. Start the remote server in one terminal:
    /usr/local/bin/python3 06.research_server_sse.py

    # 2. Run this client in another terminal:
    /usr/local/bin/python3 06.chat_bot_remote_server.py

The client reads server_config_remote.json.
Each server entry must have:
    - "url"       : the server endpoint
    - "transport" : "sse" | "streamable-http"

Key difference from stdio client (03/04/05):
    - stdio  → client launches the server as a subprocess (command + args)
    - remote → server runs independently; client connects to its URL
"""
...
# Uncomment if you use Streamable HTTP transport instead of SSE:
...
class MCP_ChatBot:
    ...
    async def connect_to_server(self, server_name: str, server_config: dict) -> None:
        """
        Connect to a remote MCP server over SSE or Streamable HTTP.

        server_config keys:
            url       – full server endpoint, e.g. http://localhost:8001/sse
        ...
        try:
            if transport == "sse":
                # SSE: persistent connection, server pushes events to client
                raw_transport = await self.exit_stack.enter_async_context(
                    sse_client(url=url)
                )
            elif transport == "streamable-http":
                # Streamable HTTP: newer transport, URL typically ends with /mcp/
                from mcp.client.streamable_http import streamablehttp_client
                raw_transport = await self.exit_stack.enter_async_context(
                    streamablehttp_client(url=url)
                )
            else:
                print(f"Unknown transport '{transport}' for server '{server_name}'")
                return

            read, write = raw_transport
            ...
            print(f"Connected to '{server_name}' ({transport}) at {url}")
            print(f"  tools: {[t.name for t in tools_response.tools]}", end="")
            ...
            try:
                ...
                uris = [str(r.uri) for r in resources_response.resources]
                print(f"\n  resources: {uris}", end="")
            ...
            try:
                ...
                names = [p.name for p in prompts_response.prompts]
                print(f"\n  prompts: {names}", end="")
    ...
    async def connect_to_servers(self) -> None:
        """Read server_config_remote.json and connect to all configured remote servers."""
        try:
            with open("server_config_remote.json", "r") as f:
                data = json.load(f)
        ...
        while True:
            try:
                ...
                if not query:
                    ...
                    await self.get_resource(query[1:].strip())
                elif query == "/prompts":
                    self.list_prompts()
